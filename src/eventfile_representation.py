...
class EtMap:
    ...
    def visualize(self, figsize=(5,5), cmap='viridis', lognorm=False):
        """
...
class EtdtCube:
    ...
    def visualize(self, figsize=(5, 5), cmap='viridis', lognorm=False):
        ...
        ax.set_ylabel(r'$\tau$')
        ax.set_zlabel(r'$\epsilon$', labelpad=-1)
        
        # ax.set_title(r'$E-t-dt$ Cube')
    ...
    def visualize_projections(self, figsize=(10, 10), cmap='viridis', lognorm=False):
        """
        Visualize the energy-time-delta time cube.
        ...
        ax3.set_ylabel(r'$\epsilon$', labelpad=-5)
        ...
        dt_mesh = np.ravel(dt_mesh)
        Etdt_mesh = np.ravel(self.etdt_cube)
        ax4.scatter(dt_mesh, t_mesh, E_mesh, s=5*np.log(Etdt_mesh*1000), alpha=0.9, edgecolors='k',
                    c=Etdt_mesh, cmap=cmap, norm=LogNorm() if lognorm else None, linewidths=0.4)
